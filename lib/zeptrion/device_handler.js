...
module.exports = class Device_Handler{
    ...
    static set_device(adapter, device_info){
        ...
        adapter.log.debug('set_device: name: ' + name + ' channel_addr: ' + addr + ' channel_addr: ' + port)
    ...
    static stop_websocket(id, adapter){
        Websocket.stop_websocket(id, adapter)
    }
    ...
    static renew_all_websockets(adapter){
        adapter.getDevices(function(err, device_objects){
            ...
            device_objects.forEach(device_object => {
                try {
                    Websocket.stop_websocket(device_object._id, adapter)
                    Device_Handler._add_to_websocket(device_object._id, adapter) 
                } catch (error) {
                    adapter.log.error('Error renew websocket: ' + device_object._id)
