...
module.exports = class Websocket{
        ...
        ws.on('close', function close() {
            ...
            Object.keys(websockets).map(function(key, index) {
                if(websockets[key].addr + ':' + websockets[key].port === ip){
                    ...
                    websockets[key].adapter.log.debug('reopen: channel_id: ' + key + ' channel_addr: ' + websockets[key].addr + ' channel_port: ' + websockets[key].port);
    ...
    static stop_websocket(channel_id, adapter){
        if(channel_id in websockets){
            console.log('stop_websocket: channel_id: ' + key + ' channel_addr: ' + websockets[channel_id].addr + ' channel_port: ' + websockets[channel_id].port);
            adapter.log.debug('stop_websocket: channel_id: ' + key + ' channel_addr: ' + websockets[channel_id].addr + ' channel_port: ' + websockets[channel_id].port);
